...
router.get("/stats", async(req, res)=>{
    try{
        const [totalUser, totalProducts, totalOrders, recentOrder, topSellingProducts] = await Promise.all([
            User.countDocuments(),
        ...
        res.status(200).json({totalOrders,totalProducts, totalUser, pendingDelivery,revenue30days:revenue30days[0]?.revenue || 0, recentOrder, topSellingProducts})
