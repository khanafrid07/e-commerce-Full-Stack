...
export default function Checkout() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (selectedAddress) {
        ...
        const res = await dispatch(addAddress(formDetails));
  ...
  const shipping = 0;
  const discount = 0;
  let recalculatedTotal = cartData.items.reduce((sum, item) => {
          ...
          {defaultAddresses.length > 0 && (
            <div className="mb-4">
              ...
              {defaultAddresses.map((addr, i) => (
                <div
                  key={i}
                  className={`p-3 border rounded-lg mb-2 cursor-pointer ${selectedAddress === addr ? "border-green-500 bg-green-50" : "border-gray-200"
                    }`}
                  onClick={() => {
