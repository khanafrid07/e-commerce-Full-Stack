...
export default function ProductCard({
  ...
  const discountedPrice = product?.basePrice * (product?.discount/100)
  const finalPrice = product?.basePrice - discountedPrice

  return (
    <div
      onClick={() => navigate(`/products/${product._id}`)}
      className="group cursor-pointer w-full max-w-[12rem] md:max-w-[18rem] mx-auto 
                 rounded-t-xl overflow-hidden bg-gray-100 shadow-sm 
                 hover:shadow-xl transition duration-300"
    >
      {/* IMAGE */}
      <div className="relative w-full h-[11rem] sm:h-[13rem] md:h-[16rem] overflow-hidden">
        <img
          src={image}
          alt={product?.title}
