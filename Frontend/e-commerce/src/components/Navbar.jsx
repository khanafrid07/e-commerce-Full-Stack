...
export default function Navbar({ wishCount = 0 }) {
  ...
  const { cartCount } = useDashboardStats();
  ...
  return (
    <>
      {/* ── NAVBAR (fixed so it always sits above ALL page stacking contexts) ── */}
      <nav className={`fixed top-0 left-0 right-0 z-[9999] border-b transition-all duration-300 ${scrolled ? "bg-white/95 backdrop-blur-md shadow-md" : "bg-white"
        }`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-2 flex items-center gap-4 justify-between">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Link to="/" className="flex items-baseline text-2xl font-black tracking-tight select-none">
            ...
            {showSugg && suggestions.length > 0 && (
              <div className="absolute top-[calc(100%+8px)] left-0 right-0 bg-white rounded-2xl shadow-xl border border-gray-100 py-2 max-h-72 overflow-y-auto z-50">
                {suggestions.map((p) => (
                  <div
                    key={p._id}
                    onClick={() => { navigate(`/products/${p._id}`); setSearchQuery(""); setShowSugg(false); }}
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs font-bold text-purple-500">${p.price}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="flex items-center gap-4 sm:gap-5 shrink-0">
            ...
            {user ? (
              <div className="relative hidden sm:flex" ref={profileRef}>
                ...
                {/* Dropdown — fixed nav means this always paints above everything */}
                {menuOpen && (
            ...
            <div className="p-4 border-t border-gray-100">
              {user ? (
                <div>
                  ...
                  <button onClick={() => { dispatch(logout()); setMobileOpen(false); }}
